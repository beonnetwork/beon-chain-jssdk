...
class BeOnSDK {
  ...
  async transfer({
    ...
    while (found < 0 && nTries < 3) {
      ...
      found = block.transactions.findIndex(_tx => {
        console.log("compare", _tx, txData);
        return _tx === txData;
      });
      nTries += 1;
    }
    console.log("found", found, nTries);
    if (found < 0) {
    ...
    console.log("hash", this.web3.utils.soliditySha3(tx.hash(), root));
    console.log("prefixed hash", confirmationHashed);
  ...
  _confirmSig(txHash, root, key) {
    console.log("confirmSigHash", txHash, root, Web3.utils.soliditySha3(txHash, root));
  ...
  async withdraw({
    ...
    // console.log("data.utxoPos", data.utxoPos);
